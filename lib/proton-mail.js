...
class ProtonMail {
   ...
   * @param {Object} config.puppeteer
   * @param {Object} config.launchOptions
   * @return {ProtonMail}
   */
  ...
  async _connect() {
    if (this._browser === undefined) {
      this._browser = await this._config.puppeteer.launch(
        this._config.launchOptions || { headless: true }
      );
    ...
    await Promise.race([
      ...
      page
        .waitForSelector(".proton-notification-template.notification-danger ")
        .then(() => {
          invalidLogin = true;
        }),
    ]);
